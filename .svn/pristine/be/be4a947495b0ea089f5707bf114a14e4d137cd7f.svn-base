...
(function($) {
  ...
  FsScroll.prototype = {
    ...
    _initEvent: function() {
       ...
       */
      self.element.on('mousewheel DOMMouseScroll touchmove touchstart touchend', function(e) {
		var event = event || window.event;
		var distance,direction,clientY_start, clientY_end, minRange = 10;
		this.clientY_start;
		switch (event.type) {
			case "touchstart":
      ...
      if(self.options.keyboard) {
		...
		/*时间间隔*/
		if(self.options.Intervaltime) {
			setInterval(function(){self.next(); } ,self.options.Intervaltime);
		}
