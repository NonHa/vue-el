...
export default {
  ...
  methods: {
    getCheckCode() {
      if (this.phoneStyle) {
         if (this.flag) {
         getUpdateBindRealTelCheckCode().then(res => {
          if (res.Code == 0) {
    ...
    sureLogin() {
      this.phoneForm.operate = 'doUpdateBindRealTel'
      doUpdateBindRealTel(this.phoneForm).then(res => {
        if (res.Code == 0) {
          this.$emit('success')
           this.$toast(res.Msg);
        } else {
  ...
  computed: {
    phoneStyle() {
        ...
        if (!reg.test(this.phoneForm.update_tel)) {
            return false
        }
        return true
    },
